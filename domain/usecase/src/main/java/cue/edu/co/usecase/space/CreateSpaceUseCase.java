...
public class CreateSpaceUseCase {
    ...
    public Space execute(CreateSpaceCommand command) {
        validateUniqueName(command.name(), command.campusId());
    ...
    private void validateUniqueName(String name, Long campusId) {
        if (spaceRepository.existsByNameAndCampusId(name, campusId)) {
